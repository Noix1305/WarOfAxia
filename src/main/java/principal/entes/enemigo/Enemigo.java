 ...
 */
package principal.entes.enemigo;
...
public class Enemigo extends Entidad {
    ...
    private void atacar(Jugador jugador) {
        ...
        if (getArea().intersects(ElementosPrincipales.jugador.areaPosicional)) {
            ...
            if (tiempoActual - tiempoUltimoAtaque >= TIEMPO_ENTRE_ATAQUES && getArea().intersects(jugador.areaPosicional)) {
                jugador.perderVida(gestorAtributos.getAtaqueEnemigo());
    ...
    public boolean colisionConObjeto(double nuevaX, double nuevaY, Rectangle r) {
        ...
        return areaEnemigo.intersects(r);
    }
    ...
    public void idlePosition(Graphics g, Rectangle posMenu, int id) {
        ...
        if (gestorAtributos.getIdEnemigo() == id) {
            animacion++;
            int animacionBase = 500;

            if (animacion <= animacionBase / 2) {
                estado = 0;
            } else if (animacion <= animacionBase) {
                estado = 2;
            } else {
                animacion = 0;
            }
    ...
    private void dibujarVidaActual(final Graphics g, final int puntoX, final int puntoY) {
        DibujoDebug.dibujarString(g, "" + Float.toString(gestorAtributos.getVidaEnemigo()), puntoX, puntoY - 8);
    }
    ...
    public void perderVida(float danhoRecibido, boolean critico) {
        ...
        if (gestorAtributos.getVidaEnemigo() - (int) danhoRecibido < 0) {
            // Si la vida resulta ser menor que cero, establece la vida en cero y otorga experiencia al jugador
            gestorAtributos.setVidaEnemigo(0);
            ElementosPrincipales.jugador.getGestorAt().setExperiencia(ElementosPrincipales.jugador.getGestorAt().getExperiencia() + 10);
        } else {
            // De lo contrario, reduce la vida del enemigo
            gestorAtributos.setVidaEnemigo(gestorAtributos.getVidaEnemigo() - (int) danhoRecibido);
        }
    }
